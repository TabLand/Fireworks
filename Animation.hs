...
module Animation (
        ...
        -- * Time-varying values
        Varying,
        ever, onceSteps, onceSmooth, repeatSteps, repeatSmooth,
        -- ** Special cases
...
data Varying v
        ...
        | Varying v [(Time, v)] Change AtEnd
                -- ^ @'Varying' v0 [(t1, v1), ..., (tn, vn)] c e@ is a
                -- value that is initially @v0@, then changes to @v1@
                -- at time @t1@ and so on until time @tn@.
...
cycleSteps t (v:vs) =
        repeatSteps (head vs)
                [(fromIntegral n*t, v) | (n, v) <- zip [0..] (v:vs++[v])]
...
animate attrName (Varying v steps change atend) =
  ...
  where attrs = ("attributeName", attrName) :
                atEndAttr atend :
                animateAttrs v steps change
...
animateAttrs :: String -> [(Time, String)] -> Change -> [Attribute]
animateAttrs v steps change =
                ("begin", showNumber begin) :
                ("dur", showNumber duration) :
                ("keyTimes", mkList [showNumber ((t-begin)/duration) | (t, v) <- steps]) :
                ("values", mkList [v | (t, v) <- steps]) :
                changeAttrs change
  where begin = fst (head steps)
...
rect w h = Anim (element "rect" [w_attr, h_attr] (w_anim ++ h_anim))
  where (w_attr, w_anim) = animate "width" (mapVarying showNumber w)
...
ellipse rx ry =
  ...
  where (rx_attr, rx_anim) = animate "rx" (mapVarying showNumber rx)
...
withGenPaint colour opacity (Anim pic) =
  ...
  where (f_attr, f_anim) = animate "fill" (mapVarying showColour colour)
...
-- shapes whose colour has not already been specified with colour @c@.
--
-- (equivalent to @'withGenPaint' c ('ever' 1) pic@)
...
-- fully transparent.
--
-- (equivalent to @'withGenPaint' 'black' ('ever' 0) pic@)
...
withGenBorder colour opacity thickness (Anim pic) =
  ...
  where (s_attr, s_anim) = animate "stroke" (mapVarying showColour colour)
...
--
-- (equivalent to @'withGenBorder' c t ('ever' 1) pic@)
...
--
-- (equivalent to @'withGenBorder' c ('ever' 0) ('ever' 1) pic@)
...
translate p pic = transform "translate" (mapVarying showPoint p) pic
  where showPoint (x,y) = showNumber x ++ "," ++ showNumber y

-- | @'scale' s pic@ is a picture obtained by scaling @pic@ by a factor @s@.
-- Thus @scale ('ever' (2, 2)) pic@ is twice as big,
-- and @scale ('ever' (0.5, 0.5)) pic@ is half the size.
...
hsl h s l
  ...
  | otherwise = RGB r g b
  where chroma = (1 - abs (2*l - 1)) * s
        component c = (min 1 (max 0 (2 - 6*abs (snd (properFraction (c/360 + 1/2)) - 1/2)))) * chroma - chroma/2 + l
        r = component h
        g = component (h + 240)
        b = component (h + 120)
...
showColour (RGB r g b) = "#" ++ showComp r ++ showComp g ++ showComp b
  where showComp x
          | length hex == 1 = "0" ++ hex
          | otherwise = hex
          where hex = showHex (round (255*x)) ""

showNumber :: Double -> String
showNumber x
  | fromIntegral n == x = show n
  | otherwise = showFFloat (Just 6) x ""
  where n = floor x
...
--
-- > triangle :: Animation
-- > triangle = withPaint (ever (hsl 0.4 0.5 0.25))
...
-- > roundel :: Animation
-- > roundel =
-- >     translate (ever (100, 75))
-- >         (withBorder (ever red) (ever 20)
-- >             (withoutPaint (circle (ever 50))))
-- >     `plus`
-- >     translate (ever (25, 62.5))
-- >         (withGenPaint (ever blue) (ever 0.75)
-- >             (rect (ever 150) (ever 25)))
--
-- <<roundel.svg>>
--
...
-- >             (circle (ever 20)))
-- >    `plus`
-- >    withBorder (ever navy) (ever 10)
-- >        (withoutPaint
-- >            (translate (ever (30, 30)) (rect (ever 340) (ever 190))))
--
-- <<bouncing.svg>>
